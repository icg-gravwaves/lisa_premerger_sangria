...
for bank_idx in tqdm(np.arange(bank_array.size)):
    ...
    # Roll it back so that the peak is at the right 
    # amount of time before merger
    snr = snr.cyclic_time_shift(-cutoff_s)
    snr._epoch = -args.data_length
...
ideal_snr = ideal_snr.cyclic_time_shift(-cutoff_s)
ideal_snr._epoch = -args.data_length

fig, ax = plt.subplots(1)
print(2 * np.sqrt(2))
print(fitting_factor_over_time.max())
print(max(ideal_snr))
...
fig.savefig("fitting_factor_vs_time_TEST.png")