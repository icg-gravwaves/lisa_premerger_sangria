...
To make the movie, we use `make_images.py`, which uses a generated signal from our previous paper (downloaded from that repo). We steap through the data in 0.1 day steps, and gate / inpaint from the end of the data through to the end of the timeseries. We add gaps if appropriate.
